...
export async function updateInventory(ctx: Context, next: () => Promise<void>) {
  ...
  const payload: RequestPayloadLogistics = await json(ctx.req)
  ...
  if (!payload.hasOwnProperty('quantity') || !(typeof payload.quantity === 'number')) {
    ...
    try {
      ...
      await logistics.updateInventoryBySkuAndWarehouse(skuId, String(warehouseId), payload, customHeaders)
